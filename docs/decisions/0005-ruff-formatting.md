...
the tool `ruff` in order to replace `black` and `isort`. Whilst we do not have any major
...
can impact existing codebases. This is similar to the impact that adding `black` to an
existing product can have. For new codebases the impact is minimal.
...
the single new dependency `ruff`. Ultimately this results in fewer dependencies to keep
on top of.

The use of [invoke][invoke] to wrap our common local development workflows, and reuse
them in our GitHub action flows pays dividends here. The adoption of `ruff` for code
