# 0007 Adopt Copier for Project Generation
...
represented the simplest route to getting the template project up and running.
...
after it was created. Building and maintaining such a tool is entirely possible – and
likely quite fun. But there are also several tools in the Python ecosystem already that
are targeted at this kind of functionality. Namely, [Cookiecutter][cookie] and more
recently [Copier][copier].

Cookiecutter has been the _de facto_ approach for a while, and it works quite well. It
...
killer feature and makes one of the problems with template projects less of a concern –
that of keeping existing projects in sync. With the more traditional one and done
approach of Cookiecutter, once the creation is done, changes in the base template
have to be manually applied to an existing project. Or worse, they are applied in a single
...
weird in the template codebase, but it also makes it quite explicit.

There is a third option: write our own tool designed to work with our own project
structures that either replaces or complements `startproject`. This approach would be
fun but would likely require significant time investment to create and more to maintain.
It would also likely result in a tool with fewer features than the existing tools
available.
...
The outcome of choosing Copier is a template that initially works very similarly to how
